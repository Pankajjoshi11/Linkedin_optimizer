    ...
    try {
      ...
      if (analysisData.rewritten && Object.keys(analysisData.rewritten).length > 0) {
        ...
        try {
          ...
          if (rewriteError.message && (rewriteError.message.includes('quota') || rewriteError.message.includes('429'))) {
            console.log('API quota/rate limit exceeded, proceeding with original content');
            quotaExceeded = true;
          } else {
            console.log('Other error occurred, proceeding with original content');
          }
        }
      }
      ...
      try {
        ...
        // Add a custom header to indicate quota status
        if (quotaExceeded) {
          res.setHeader('X-Quota-Warning', 'API quota exceeded - using original content');
        }
        ...
        throw new Error(`PDF generation failed: ${methodError.message}`);
      }
